...
\begin{itemize}
  \item Dados resultantes da comparação entre cinco diferentes modelos de linguagem e seu desempenho na extração de contextos relevantes presentes em dados de laboratórios e demandas coletados pelo \gls{direc} ao longo dos anos.
  \item A criação de uma aplicação capaz de integrar diferentes modelos de linguagem para a análise linguistica de textos e extração de palavras-chave, bem como a análise de similiaridade entre contextos.
...
Para realizar a comparação entre os modelos integrados foi construído um projeto a parte de teste e benchmarking. Este projeto simula o consumo dos pontos de extremidade expostos pela \gls{api} de aplicação da mesma forma que o aplicativo mobile, porém obtendo informações relevantes para a análise de desempenho, como por exemplo, o tempo de análise de cada modelo, armazenando as informações obtidas em um banco de dados. Estas informações são utilizadas na conclusão e ajudam a enteder os pontos positivos e possíveis melhorias que podem basear trabalhos futuros.

% ----------------------------------- Modelagem do Sistema ---------------------------------- %

\section{Modelagem do sistema}\label{sec:modelagem}

O processo de modelagem do sistema foi divido de modo a habilitar a compartimentalização através da separação de conceitos e tarefas que cada módulo deve atender, permitindo uma maior escalibidade e facilitando o processo de manutenção. Nesta seção estão identificados os requisitos de cada aplicação e o projeto arquitetural de cada uma.
...
A \gls{api} de aplicação foi projetada para que possa ser integrada a qualquer sistema que seja capaz de realizar requisições \gls{http} através de uma arquitetura \gls{rest}. Também foi configurada para ser o agente responsável pela manipulação e persistência dos dados. As tabelas \autoref{tab:requisitos_funcionais_api} e \autoref{tab:requisitos_nao_funcionais_api} apresentam os requisitos indentificados para esta aplicação.
...
\begin{figure}[H]
  ...
  \label{fig:ERD}
  \includegraphics[scale=0.22]{ERD}
  \fonte{}
\end{figure}
...
\begin{table}[H]
  \caption{Requisitos funcionais da api de linguagem.}
...
\begin{itemize}
  ...
  \item \textbf{extract}: Responsável pela extração de palavras-chave de um texto utilizando um modelo específico.
\end{itemize}
...
O sistema possui uma tela para entrar inicialmente, onde o usuário pode preencher seu email e senha, ou fazer seu registro. Antes de se cadastrar, o usuário precisa selecionar se ele é um laboratório ou organização, e então ele é direcionado para seu respectivo formulário para preencher com suas informações. As telas para entrar e se cadastrar no sistema estão representadas na \autoref{fig:pagina-login-signin}.
...
Ou seja, uma organização poderá ver seu perfil, as demandas e os ranqueamentos de todos os laborátorios para cada demanda, além dos seus detalhes, enquanto o laboratório poderá ver seu perfil e os matches com os detalhes da demanda do organizador que o marcou como favorito.
...
A \gls{api} de aplicação possui quinze rotas organizadas em cinco subdomínios. O ponto inicial de interação de um usuário com a aplicação são as rotas de registro, tanto para laboratórios quanto para organizações.
...
A rota \texttt{/register} espera um corpo de requisição como descrito no códido do anexo \ref{codigo:register-lab}.

O consumo deste endpoint engatilha a chamada de extração de palavras-chave utilizando o modelo informado no campo \textit{model}, aceitando os valores \textit{Gpt, Aws, Bert, Yake e Azure}. Este campo é opcional e quando não informado utilizará o modelo \gls{bert} por padrão. O código do anexo \ref{codigo:analyze-lab} representa o método que recebe a requisição, a deserializa e chama o endpoint da \gls{api} de linguagem para realizar a extração. Este método também realiza algumas validações antes de prosseguir com o processamento, como a garantia de que o laboratório com o email informado já não esteja cadastrado, de que a senha informada possua pelo menos 8 caracteres com pelo menos um número e uma letra e que o campo descrição, utilizado na extração de contexto, esteja preenchido.

É importante notar que na chamada do método é permitido ao usuário informar uma lista de palavras-chave. Essas palavras-chave são agrupadas às extraídas pelo modelo e armazenadas no banco de dados, para que sempre que necessário possam ser utilizadas na análise de similaridade.

O subdomínio de organizações também possui três rotas:
...
O registro de organizações não engatilha a extração de palavras-chave, pois para usuários do tipo organização isso só ocorre na criação ou atualização de uma demanda.
...
O método \texttt{/like} é utilizado por uma organização para marcar um laboratório como possível candidato para uma demanda. O método \texttt{/list} pode ser utilizado por ambos os tipos de usuários e de algumas maneiras. Este método aceita algumas informações de filtro na query, sendo eles:
...
\begin{itemize}
  \item \texttt{/match/list}
        \begin{itemize}
          ...
          \item Quando chamada por um usuário do tipo organização, retorna todas as demandas da organização, contendo todos os laborátorios e seus respectivos scores para cada demanda.
        \end{itemize}
  \item \texttt{/match/list?demand=id}: Onde id é uma demanda específica.
        \begin{itemize}
          ...
          \item Quando chamada por um usuário do tipo organização, retorna a demanda da organização, contendo todos os laborátorios e seus respectivos scores.
        \end{itemize}
  \item \texttt{/match/list?laboratory=id}: Onde id é um laboratório específico.
        \begin{itemize}
          \item Quando chamado por um usuário do tipo laboratório, retorna todas as demandas que foram marcadas como favoritas para ele.
          \item Quando chamada por um usuário do tipo organização, retorna as demandas da organização, contendo o laborátorio e seus respectivos scores para cada demanda.
        \end{itemize}
  \item \texttt{/match/list?demand=id\_demand\&laboratory=id\_lab}: Onde \texttt{id\_demand} é uma demanda específica e \texttt{id\_lab} é um laboratório específico.
        \begin{itemize}
          ...
          \item Quando chamada por um usuário do tipo organização, retorna a demanda da organização, contendo o laborátorio e seu respectivo score para a demanda.
        \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item \texttt{LANGUAGE}: Alguns dos modelos possibilitam informar qual a linguagem do texto sendo informado, o que pode ajudar na precisão do resultado, configurada como \texttt{pt} ou \texttt{portuguese}.
  \item \texttt{THRESHOLD}: O valor mínimo de peso para que uma palavra seja considerada relevante, configurada como 0.6.
\end{itemize}

O método do códido \ref{codigo:extract-aws} é responsável por realizar a chamada à \gls{api} da Amazon Web Services ao modelo do Amazon Comprehend. É utilizado o pacote \textbf{botocore}, desensolvido e mantido pela própria Amazon Web Services:

\begin{sourcecode}[H]
  \caption{\label{codigo:extract-aws}Método de extração de palavras-chave utilizando o modelo da AWS}
...
\begin{sourcecode}[H]
  \caption{\label{codigo:extract-azure}Método de extração de palavras-chave utilizando o modelo da Microsoft}
  \begin{lstlisting}[frame=single, language=Python]
...
O método do códido \ref{codigo:extract-yake} é responsável por realizar a extração utilizando o método \gls{yake}. É utilizado o pacote \textbf{yake} \cite{LiaadYake2023}. Este método não realiza nenhuma chamada à \gls{api} externas, executando todo o processamento localmente.

\begin{sourcecode}[H]
  \caption{\label{codigo:extract-yake}Método de extração de palavras-chave utilizando o método YAKE}
  \begin{lstlisting}[frame=single, language=Python]
...
\begin{sourcecode}[H]
  \caption{\label{codigo:extract-gpt}Método de extração de palavras-chave utilizando o modelo GPT}
  \begin{lstlisting}[frame=single, language=Python]
...
Por fim, o códido \ref{codigo:extract-bert} é responsável por realizar a extração utilizando o modelo da Google \gls{bert}. Foi utilizado o pacote \textbf{keybert} \cite{grootendorst2020keybert}.

\begin{sourcecode}[H]
  \caption{\label{codigo:extract-bert}Método de extração de palavras-chave utilizando o modelo BERT}
  \begin{lstlisting}[frame=single, language=Python]
...
\begin{itemize}
  \item \texttt{Score}: Pontuação do modelo para o par demanda e laborátorio.
  \item \texttt{Time}: Tempo em milisegundos na extração de palavras-chave do laboratóroio pelo modelo.
  \item \texttt{WordCount}: Número de palavras contidas na demanda.
  \item \texttt{KeywordCount}: Número de palavras-chaves extraidas pelo modelo para cada laborátorio.
  \item \texttt{Status}: Indica se o modelo foi capaz de extrair as palavras-chave.
\end{itemize}

Os gráficos das figuras \ref{fig:pontuacoes} e \ref{fig:distribuicao} apresentam a pontuação média de cada modelo e a distribuição de pontuações para todo o conjunto de dados.

\begin{graph}[H]
  \caption{Gráfico da pontuação média por modelo}
  \includegraphics[scale=0.65]{pontuacoes}
  \fonte{Autoria própria (2025)}
...
\begin{graph}[H]
  ...
  \includegraphics[scale=0.65]{distribuicao}
  \fonte{Autoria própria (2025)}
  \label{fig:distribuicao}
\end{graph}

Todos os modelos apresentaram um desempenho similiar, com a maioria das pontuações entre 0.2 e 0.3, mas apresentando casos de pontuações mais altas e mais baixas, o que pode indicar um melhor match entre demanda e laboratório para os casos específicos como pode ser visto no gráfico da figura \ref{fig:top_scores_mean} apresentando a média de pontuações máximas por modelo.

\begin{graph}[H]
  \caption{Gráfico da média de pontuações máximas por modelo}
  \includegraphics[scale=0.65]{top_scores_mean}
  \fonte{Autoria própria (2025)}
  \label{fig:top_scores_mean}
\end{graph}

É importante destacar que para se obter uma pontuação 1 na análise de similaridade, é necessário que ambos os textos sendo comparados sejam idênticos, portanto resultados próximos ou acima de 0.5, como os obtidos indicam que estas seriam as pontenciais melhores indicações para uma avaliação mais detalhada de parceria entre demanda e laboratório.

Foram plotados também os gráficos de tempo médio de processamento e número médio de palavras-chave extraídas por modelo, apresentados nas figuras \ref{fig:tempo} e \ref{fig:palavras-chave}.

\begin{graph}[H]
  \caption{Gráfico do tempo médio de processamento por modelo}
  \includegraphics[scale=0.65]{tempo}
  \fonte{Autoria própria (2025)}
  \label{fig:tempo}
\end{graph}

\begin{graph}[H]
  \caption{Gráfico do número médio de palavras-chave extraídas por modelo}
  \includegraphics[scale=0.65]{palavras-chave}
  \fonte{Autoria própria (2025)}
  \label{fig:palavras-chave}
\end{graph}

Como esperado, o método \gls{yake} possui os menores tempos de processamento pois se trata de um modelo estatístico que é executado localmente, porém vale ressaltar a bom desempenho de tempo dos modelos AWS e Azure,

Plotou-se também o gráfico da relação entre a pontuação média de cada modelo em relação
...
\begin{graph}[H]
  ...
  \includegraphics[scale=0.65]{pontuacao-palavras}
  \fonte{Autoria própria (2025)}
  \label{fig:pontuacao-palavras}
\end{graph}

Este gráfico destaca uma caraterística importante na análise, um fraco vínculo entre o número de palavras-chave extraídas e a pontuação média. Também pode-se notar o bom desempenho do método estatístico \gls{yake}, que mesmo não sendo uma ferramenta robusta como as demais \gls{llm}'s, apresentou um bom desempenho em comparação.

O gráfico da figura \ref{fig:erros} apresenta o percentual de erros por modelo durante o processamento. São considerados erros todos os processamentos em que um modelo não foi capaz de extrair palavras-chave.

\begin{graph}[H]
  \caption{Gráfico do percentual de erros por modelo}
  \includegraphics[scale=0.65]{erros}
  \fonte{Autoria própria (2025)}
  \label{fig:erros}
\end{graph}

Como pode ser visto, somente o modelo \gls{bert} apresentou erros no processo de extração. Estes erros ocorreram para 9 dos 78 laboratórios, e em todos os casos o erro foi causado pelo tamanho da descrição do laboratório, sendo muito pequena para que o modelo fosse capaz de extrair palavras-chave.

Por fim, para melhor compreensão dos resultados, foi plotado o gráfico do radar de desempenhos dos modelos por métrica utilizada, apresentado na figura \ref{fig:radar}.

\begin{graph}[H]
  \caption{Radar do desempenho dos modelos por métrica}
  \includegraphics[scale=0.65]{radar}
  \fonte{Autoria própria (2025)}
  \label{fig:radar}
\end{graph}

Vale notar que na visualização na forma de radar, quanto mais próximo das bordas do círculo, melhor é o desempenho do modelo para a métrica em questão, portanto foi invertido o eixo da métrica de tempo de processamento para melhor representar os resultados.