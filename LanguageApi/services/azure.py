...
class AzureService:
    ...
    def extract(self, description: Description) -> List[Keyword]:
        return [Keyword(text=keyword, weight=Settings.keyword_threshhold)
                for keyword in self.client.extract_key_phrases([description.text], language=Settings.language)[0].get("key_phrases")]
