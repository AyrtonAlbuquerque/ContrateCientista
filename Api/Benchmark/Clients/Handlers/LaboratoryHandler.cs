...
{
    public class LaboratoryHandler(IConfiguration configuration, HttpClient client, IMemoryCache memoryCache) : DelegatingHandler
    {
        ...
        {
            if (!memoryCache.TryGetValue("LabAuth", out Auth auth))
            {
                var request = JsonSerializer.Serialize(new Login
                {
                    Email = configuration["Api:LabEmail"],
                    Password = configuration["Api:LabPassword"]
                });
                ...
                memoryCache.Set("LabAuth", auth, new MemoryCacheEntryOptions
                {
